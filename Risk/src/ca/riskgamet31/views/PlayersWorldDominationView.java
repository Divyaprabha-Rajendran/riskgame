...
public class PlayersWorldDominationView implements Observer
  {
	...
	private void calculateWorldDominationInfo(GameMainDriver gmd)
	{
	  ...
	  ArrayList<GraphNode> mapGraphNodes = gmd.getGameMap().getGameMapGraph().getGraphNodes();
	 ...
	 Map<String, String> countriesArmiesPerPlayer= mapGraphNodes.stream().map(i -> i.getNodeData())
			.collect(Collectors.groupingBy(Country::getCurrentOccupier,Collectors.collectingAndThen(Collectors.summarizingInt(Country::getArmies), s -> String.format("%.2f%%    %d", ((float)s.getCount()*100/(float)countryCount),s.getSum()))));
	 
	for (Map.Entry<String, String> mapEntry : countriesArmiesPerPlayer.entrySet())		
	  {
		
			for (Continent continent : gmd.getGameMap().getContinentsList().values())
			  {
				
				if ((continent.getContinentGraph().getGraphNodes().stream().map((
				    c) -> c.getNodeData().getCurrentOccupier()).anyMatch((o) -> !o
				        .equals(mapEntry.getKey()))))
				  {
