...
public class PhaseView extends Observable implements Observer
  {
	...
	/**
	 * to display phase information, this method is called inside the update
	 * method.
	 */
	...
	public void update(Observable o, Object arg)
	  {
		...
		if (phaseName.trim().startsWith("Reinforcement Phase"))
		  {
			Optional<Player> currentPlayerOptional = ((GameMainDriver) o)
			    .getPlayerList().getPlayerList().stream().filter(x -> x
			        .getplayerName().equals(this.playerName)).findFirst();
			Player currentPlayer = currentPlayerOptional
			    .isPresent() ? currentPlayerOptional.get() : null;
