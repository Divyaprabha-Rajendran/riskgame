	...
	public boolean isEligibleToExchange()
	  {
		...
		Map<String, Integer> handStats = this.getCardsFromHand().stream()
		    .collect(Collectors.groupingBy(i -> i.getCardType(), Collectors
		        .reducing(0, e -> 1, Integer::sum)));
		
		numOfTypes = handStats.size();
		maxPerType = handStats.values().stream().max((x, y) -> x.intValue() - y
		    .intValue()).orElse(0).longValue();
	...
	public boolean canTurnInCards(int index1, int index2, int index3)
	    throws IndexOutOfBoundsException
	  {
		...
		if (handWithCards.size() >= 3)
		  {
			if (handWithCards.get(index1).getCardType().equals(handWithCards
			    .get(index2).getCardType()) && handWithCards.get(index1)
			            ...
			            .getCardType()))
			  {
				...
				mustTurnInCards = true;
			      ...
			      .equals(handWithCards.get(index2)
			          .getCardType()) && !handWithCards.get(index1)
			              .getCardType().equals(handWithCards.get(index3)
			                  .getCardType()) && !handWithCards.get(index2)
