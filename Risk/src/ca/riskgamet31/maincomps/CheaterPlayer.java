...
public class CheaterPlayer implements Player
  {
	 ...
	 */
	public CheaterPlayer(String playersName, int army)
	...
	/**
	 * checks if Cheater player can fortifies.
	 * 
	...
	public ArrayList<GraphNode> canFortify()
	  {
		...
		ArrayList<GraphNode> destinationCountries = new ArrayList<>(this
		...
		for (GraphNode countryNode : fortifyNodes)
		  {
			...
			countryNode.getNodeData().increaseArmies(countryNode.getNodeData()
			    .getArmies());
	...
	/**
	 * double the armies in player countries with bounded maximum of 100,000 to
	 * avoid overflow.
	 * 
	 */
	@Override
	public void reinforcement()
	  {
		System.out.println("All Country armies will be doubled for " + this
		    .getplayerName());
		
		for (GraphNode countryNode : this.getPlayerGraph().getGraphNodes())
		  {
			int reinforcementNumber = countryNode.getNodeData()
			    .getArmies() * 2 >= 100000 ? 100000 : countryNode.getNodeData()
			        .getArmies() * 2;
			countryNode.getNodeData().setArmies(reinforcementNumber);
			
		  }
		  
		System.out.println("All Countries armies are doubled for " + this
		    .getplayerName());
	...
	public boolean attack(MainDriver driver)
	  {
		...
		ArrayList<GraphNode> destinationCountries = new ArrayList<>(this
		    .getPlayerGraph().getGraphNodes().stream().filter(x -> x
		        .getNodeData().getArmies() > 1).filter(i -> i.getNodeNeighbors()
		            .stream().anyMatch(x -> !x.getNodeData()
		                .getCurrentOccupier().equals(this.getplayerName())))
		    .collect(Collectors.toList()));
		...
		for (GraphNode countryNode : destinationCountries)
		  {
			...
			ArrayList<GraphNode> otherPlayersNeighbors = new ArrayList<>(countryNode
			    .getNodeNeighbors().stream().filter(x -> !x.getNodeData()
			        .getCurrentOccupier().equals(this.getplayerName()))
			    .collect(Collectors.toList()));
			...
			for (GraphNode neighbor : otherPlayersNeighbors)
			  {
				...
				for (Player defender : driver.getPlayerList().getPlayerList())
				  {
					...
					if (defender.getplayerName().equals(neighbor.getNodeData()
					    .getCurrentOccupier()))
					  {
				...
				if (countryNode.getNodeData().getArmies() > 1)
				  {
					neighbor.getNodeData().setCurrentOccupier(this
					    .getplayerName());
					neighbor.getNodeData().setArmies(1);
					countryNode.getNodeData().setArmies(countryNode
					    .getNodeData().getArmies() - 1);
					...
					if (defendingPlayer.getPlayerCountriesGNodes().size() == 0)
					  {
						...
						System.out.println(this
						    .getplayerName() + " will receive - " + defendingPlayer
						...
						driver.getPlayerList().getPlayerList()
						    .remove(defendingPlayer);
	...
	/**
	 * check if cheater player may attack or not
	 * 
	 * @return attacking and attacked countries
	 */
	@Override
	...
	/**
	 * utility method for reinforcement main method
	 * 
	 * @return a Dummy country as cheater will double his armies
	 */
	@Override
