...
/**
 * Benevolent Players class implementing Benevolent strategy
 * 
 * @author YD
 * @version 3.0
 * @since 3.0
...
public class BenevolentPlayer implements Player
  {
	 ...
	 * 
	 * @param playersName Benevolent Player's name
	 ...
	 */
	public BenevolentPlayer(String playersName, int army)
	...
	/**
	 * checks if player can fortify
	 * 
	 * @return array list of source and destination countries.
	 */
	@Override
	public ArrayList<GraphNode> canFortify()
	  {
		...
		destinationCountries.sort((x1, x2) -> x1.getNodeData().getArmies() - x2
	...
	/**
	 * main fortification method for Benevolent player
	 */
	@Override
	...
	/**
	 * Fortification phase starting method according. fortification will be
	 * performed
	 * 
	 */
	@Override
	...
	/**
	 * Benevolent player can not attack
	 * 
	...
	public ArrayList<GraphNode> canAttack()
	  {
		...
		return Stream.of(new GraphNode(new Country("DUMMY"))).collect(Collectors
		    .toCollection(ArrayList<GraphNode>::new));
	  }
	...
	/**
	 * check if Benevolent can reinforce and returns best option
	 * 
	 * @return weakest country to reinforce
	 */
	@Override
		...
		GraphNode retNode = getPlayerGraph().getGraphNodes().stream()
		    .filter(i -> i.getNodeNeighbors().stream().anyMatch(x -> !x
		        .getNodeData().getCurrentOccupier().equals(this
		            .getplayerName()))).min((i, y) -> i.getNodeData()
